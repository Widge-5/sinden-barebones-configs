...
rp_module_id="lr-pcsx-rearmed-moorhen"
...
rp_module_repo="git https://github.com/StormedBubbles/pcsx_rearmed.git moorhen"
rp_module_section="opt arm=main"

function depends_lr-pcsx-rearmed-moorhen() {
    local depends=(libpng-dev)
    isPlatform "x11" && depends+=(libx11-dev)
    getDepends "${depends[@]}"
}

function sources_lr-pcsx-rearmed-moorhen() {
    gitPullOrClone
}

function build_lr-pcsx-rearmed-moorhen() {
...
function install_lr-pcsx-rearmed-moorhen() {
    md_ret_files=(
        'AUTHORS'
        'ChangeLog.df'
        'COPYING'
...
function configure_lr-pcsx-rearmed-moorhen() {
    mkRomDir "psx"
    defaultRAConfig "psx"
